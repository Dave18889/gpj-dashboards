...
// Manual corrections for people whose most-frequent travel region doesn't
// match their actual home base — the sheet has no explicit "home region"
// column, so this is inferred from travel frequency by default, which can
// be wrong for someone who happens to travel to a non-home region often.
// Add more entries here as they're found: 'Full Name': 'REGION'.
const HOME_REGION_OVERRIDES = {
  'Nader Bazzi': 'LATAM',
};
...
// then adjusted by HOME_REGION_OVERRIDES above for known exceptions.
function parseTeamView(rows) {
  ...
  Object.entries(counts).forEach(([name, regionCounts]) => {
    ...
    homeRegion[name] = HOME_REGION_OVERRIDES[name] || best;
  });
