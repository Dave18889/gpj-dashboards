...
// Manual corrections for conference titles that are mislabeled in the
// sheet itself — CIOME27 is a Middle East event (per its code and last
// year's equivalent, "ME CIO & IT Executive Conference"), but its title
// in the sheet was copy-pasted from the EMEA CIO Leadership Forum entry.
// Add more entries here as they're found: 'CODE': 'Corrected full name'.
const CONFERENCE_NAME_OVERRIDES = {
  CIOME27: '2027 ME CIO Leadership Forum',
};
...
function buildRecords(regionsParsed) {
  ...
  regionsParsed.forEach(({ region, currency, conferences }) => {
    conferences.forEach((conf) => {
      const correctedName = (conf.code && CONFERENCE_NAME_OVERRIDES[conf.code]) || conf.name;
      const conference = conf.code ? `${correctedName} (${conf.code})` : correctedName;
